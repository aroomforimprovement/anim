...
void mousePressed(){
  if(drawerOpen){
    float mx = mouseX - drawerX;
    for(UIButton b : buttons){
      if(b.contains(mx, mouseY)){
        b.click();
        return;
      }
    }
  }else{
    if(checkDrawerButton(mouseX, mouseY)){
      toggleMenuDrawer();
    }
  }
  if(!colourPickerOpen){
...
boolean checkDrawerButton(float mx, float my){
    float x = 0;
    float y = 0;
    float w = 20;
    float h = 20;
...
void buildDrawer(){
  ...
  buttons.add(new UIButton("Next Frame", 20, y, 240, 32, () -> next()));
  y += 40;
  buttons.add(new UIButton("Create Frames", 20, y, 240, 32, () -> createFrames()));
  y += 40;
  buttons.add(new UIButton("Set BG", 20, y, 240, 32, () -> setBg()));
  y += 40;
